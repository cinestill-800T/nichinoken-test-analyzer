...
chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
    ...
    if (request.action === "fetchTestPackage") {
        fetchTestPackageFlow(request.config)
            ...
            for (const [subjectCode, subjectInfo] of Object.entries(test.subjects)) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        // 解答・解説リンクを抽出（最初の科目ページから）
                        if (!testResult._subLinks) {
                            testResult._subLinks = extractSubLinks(doc);
                        }
                    } else {
...
// ============================================================
// 詳細ページからサブリンク（問題/解答/解説）を抽出
// ============================================================

function extractSubLinks(doc) {
    const links = {};
    doc.querySelectorAll('a[href]').forEach(a => {
        const text = a.textContent.trim();
        const href = a.href;
        if (text.includes('問題') && href.includes('open-problem')) {
            links.problemUrl = href;
        }
        if (text.includes('解答') && href.includes('sample-answer')) {
            links.answerUrl = href;
        }
        if (text.includes('解説') && href.includes('answerguide')) {
            links.commentaryUrl = href;
        }
    });
    return links;
}
