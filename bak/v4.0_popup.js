...
document.addEventListener('DOMContentLoaded', () => {
    ...
    chrome.storage.local.get(['nichinokenCollection', 'nichinokenPackageProgress'], (stored) => {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            ...
            // トップページ → パッケージモード
            if (url.includes('examination-review-top')) {
...
async function executePackageFetch() {
    ...
    if (!options.correctionList && !options.results) {
    ...
    const pollInterval = setInterval(() => {
        chrome.storage.local.get('nichinokenPackageProgress', ({ nichinokenPackageProgress: prog }) => {
            if (!prog) return;
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        chrome.tabs.sendMessage(tabs[0].id, {
            ...
            // プログレスストレージをクリア
            chrome.storage.local.remove('nichinokenPackageProgress');

            if (chrome.runtime.lastError) {
                btn.disabled = false;
                btn.textContent = '📥 パッケージをダウンロード';
                document.getElementById('pkgStatus').textContent = '⚠ 接続エラー。ページを更新してください。';
                return;
            }
            ...
            if (response?.logs?.length > 0) {
                ...
                document.getElementById('pkgDebugLog').textContent = response.logs.join('\n');
            }
            ...
            const packageData = buildPackageData(response.results);
            currentExtractedData = packageData;

            if (outputFormat === 'zip') {
                await downloadAsZip(packageData, response.results);
            } else {
...
async function downloadAsZip(packageData, results) {
    ...
    const filename = `nichinoken_${dateStr}_package.zip`;
